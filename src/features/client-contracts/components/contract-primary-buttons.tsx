...
export function ContractPrimaryButtons() {
  const { viewMode, setViewMode, setSelectedContractId } = useContracts()
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setRefreshing(false)
      toast.success('Contracts archive refreshed!')
    }, 1000)
  }
    ...
    <div className='flex items-center gap-2'>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align='end' className='rounded-xl'>
          ...
          <DropdownMenuItem onClick={() => toast.success('Signed contract list requested!')}>
            Audit Trial logs
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      >
        <Plus size={15} />
        <span className='text-xs'>Add Contract</span>
      </Button>
    </div>
  )
}
