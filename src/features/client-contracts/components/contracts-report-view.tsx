...
interface ContractsReportViewProps {
  data: Contract[]
...
const COLUMNS_CONFIG = [
  ...
  { id: 'username', label: 'Contract Code', minWidth: 120, filterable: true, sortable: true },
  { id: 'name', label: 'Contracting Client', minWidth: 150, filterable: true, sortable: true },
  { id: 'role', label: 'Agreement Type', minWidth: 150, filterable: true, sortable: false },
  { id: 'amount', label: 'Total Value', minWidth: 120, filterable: true, sortable: true },
  { id: 'validUntil', label: 'Expiry Date', minWidth: 120, filterable: true, sortable: true },
  { id: 'status', label: 'Status', minWidth: 120, filterable: true, sortable: false },
] as const

export function ContractsReportView({ data }: ContractsReportViewProps) {
    ...
    if (sortCol) {
      result.sort((a, b) => {
        ...
        if (sortCol === 'name') {
          ...
          valA = (a[sortCol as keyof Contract] as string || '').toLowerCase()
          valB = (b[sortCol as keyof Contract] as string || '').toLowerCase()
        }
  ...
  const groupedData = useMemo(() => {
    ...
    processedData.forEach((row) => {
      ...
      if (groupBy === 'name') {
        ...
        key = (row[groupBy as keyof Contract] as string) || 'No Group Specified'
      }
      ...
      <div className='relative flex flex-col flex-1 border rounded-md bg-background overflow-hidden'>
          ...
          <div className='absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 px-4 py-2 bg-popover border text-popover-foreground rounded-full shadow-lg border-primary/20 animate-in fade-in slide-in-from-bottom-2 duration-150'>
            ...
            <div className='flex items-center gap-1'>
              <Button
                ...
                onClick={() => toast.success(`Ledger reports generated for ${selectedCount} contracting clients.`)}
              >
                Send Ledger Report
              </Button>
              <Button
                      ...
                      <TableHead key='filter-status' className='px-1 py-1.5 border-r align-middle'>
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value='invited' className='text-xs'>Pending Sign</SelectItem>
                            <SelectItem value='suspended' className='text-xs'>Terminated</SelectItem>
                          </SelectContent>
                        </Select>
                      </TableHead>
                    )
        ...
        <div className='flex items-center justify-between px-4 py-2 border-t bg-muted/20 font-sans text-[11px] text-muted-foreground/75'>
          <div className='flex items-center gap-2.5'>
            <span>CAPACITY: 500 contracts</span>
...
const SpreadsheetRow = React.memo(function SpreadsheetRow({
  ...
  return (
    <TableRow className={cn('h-10 group', isSelected ? 'bg-muted/20' : '')}>
      ...
      >
        <span className='capitalize truncate'>{agreement?.label || row.role}</span>
      </TableCell>
