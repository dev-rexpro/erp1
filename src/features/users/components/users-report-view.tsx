...
interface UsersReportViewProps {
  data: User[]
  search: Record<string, unknown>
  navigate: unknown
}

// Columns definition matching users-columns.tsx
const COLUMNS_CONFIG = [
  { id: 'index', label: '#', minWidth: 45, filterable: false, sortable: false },
  { id: 'username', label: 'Username', minWidth: 120, filterable: true, sortable: true },
  { id: 'name', label: 'Name', minWidth: 150, filterable: true, sortable: true },
  { id: 'email', label: 'Email', minWidth: 180, filterable: true, sortable: true },
  { id: 'phone', label: 'Phone Number', minWidth: 140, filterable: true, sortable: true },
  { id: 'role', label: 'Role', minWidth: 120, filterable: true, sortable: false },
  { id: 'status', label: 'Status', minWidth: 120, filterable: true, sortable: false },
] as const

export function UsersReportView({ data }: UsersReportViewProps) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (colId && ['username', 'name', 'email', 'phone', 'role', 'status'].includes(colId)) {
      setGroupBy(colId)
      setCollapsedGroups({})
  ...
  const processedData = useMemo(() => {
    ...
    result = result.filter((row) => {
      ...
      const matchName = filters.name ? fullName.includes(filters.name.toLowerCase()) : true
      const matchUsername = filters.username ? row.username.toLowerCase().includes(filters.username.toLowerCase()) : true
      const matchEmail = filters.email ? row.email.toLowerCase().includes(filters.email.toLowerCase()) : true
      const matchPhone = filters.phone ? row.phoneNumber.toLowerCase().includes(filters.phone.toLowerCase()) : true
      ...
      return matchName && matchUsername && matchEmail && matchPhone && matchRole && matchStatus
    })
    ...
    if (sortCol) {
      result.sort((a, b) => {
        ...
        if (sortCol === 'name') {
          ...
          valA = (a[sortCol as keyof User] as string || '').toLowerCase()
          valB = (b[sortCol as keyof User] as string || '').toLowerCase()
        }
  ...
  const groupedData = useMemo(() => {
    ...
    processedData.forEach((row) => {
      ...
      if (groupBy === 'name') {
        ...
        key = (row[groupBy as keyof User] as string) || 'No Group Specified'
      }
  ...
  return (
    <div className='flex flex-col flex-1 gap-4 animate-fade-in'>
      ...
      {/* Pivot / Group Panel with Drag and Drop - fully aligned with Shadcn UI aesthetics */}
      <div
      ...
      <div className='relative flex flex-col flex-1 border rounded-md bg-background overflow-hidden'>
          ...
          <div className='absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 px-4 py-2 bg-popover border text-popover-foreground rounded-full shadow-lg border-primary/20 animate-in fade-in slide-in-from-bottom-2 duration-150'>
            ...
            <div className='flex items-center gap-1'>
              <Button
                ...
                onClick={() => toast.success(`Newsletter successfully prepared to trigger for ${selectedCount} accounts!`)}
              >
                Send Message
              </Button>
              <Button
        ...
        <div className='overflow-x-auto w-full'>
          <table className='w-full border-collapse table-fixed select-none text-sm'>
            ...
            <TableHeader>
              ...
              {/* SpreadSheet Filtering Row (Matches original design toolbar filters) */}
              <TableRow className='bg-[#fafafa] dark:bg-muted/10 hover:bg-muted/10 h-11 border-b'>
                ...
                {COLUMNS_CONFIG.map((col) => {
                  ...
                  // General columns text filter in sheet row
                  return (
            ...
            <TableBody className='divide-y border-b'>
              {groupBy && groupedData ? (
                ...
                // Default flat report render mode matching exactly users columns
                ...
                // Zero filters state view matches original table design guidelines 
                <TableRow>
        ...
        {/* Minimal clean spreadsheet bottom bar status indicator aligned with layout */}
...
// Spreadsheet row sub-component rendered safely with pure Shadcn theme parameters and elements
  ...
  return (
    <TableRow className={cn('h-10 group', isSelected ? 'bg-muted/20' : '')}>
      ...
      <TableCell
        ...
        className={cn(getCellClasses('index'), 'text-muted-foreground text-center font-medium bg-muted/10')}
      >
        {index}
      </TableCell>
      ...
      <TableCell
        onClick={() => selectCell('email')}
        className={cn(getCellClasses('email'), 'text-muted-foreground')}
      >
        <span className='truncate'>{row.email}</span>
      </TableCell>

      <TableCell
        onClick={() => selectCell('phone')}
        className={cn(getCellClasses('phone'), 'text-muted-foreground')}
      >
        {row.phoneNumber}
      </TableCell>
      ...
      >
        <span className='capitalize truncate'>{row.role}</span>
      </TableCell>
      ...
      >
        <Badge variant='outline' className={cn('capitalize text-[10px] py-0.5 px-2 h-5 font-normal rounded-md', badgeColor)}>
          {row.status}
        </Badge>
      </TableCell>
    </TableRow>
