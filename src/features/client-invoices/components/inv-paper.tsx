        ...
        <section className='text-sm'>
          ...
          {getInvoiceItems(invoice).map((item) => (
            ...
            >
              ...
              <span className='text-right'>{fmtCur(item.unitPrice)}</span>
              <span className='text-right'>{fmtCur(getLineAmount(item))}</span>
            </div>
          ))}
        </section>
          ...
          <section className='col-start-2 space-y-2'>
            <div>
              <div className='flex justify-between gap-8'>
                ...
                <span>{fmtCur(getInvoiceSubtotal(invoice))}</span>
              </div>
              <div className='flex justify-between gap-8'>
                <span>{discountLabel}</span>
                <span>{fmtCur(getInvoiceDiscount(invoice))}</span>
              </div>
              <div className='flex justify-between gap-8'>
                <span>
                  {taxOption.name} {taxOption.rate}%
                </span>
                <span>{fmtCur(getInvoiceTax(invoice))}</span>
              </div>
            </div>
            <div className='border-y-2 border-current py-3'>
              <div className='flex justify-between gap-8'>
                <span className='font-semibold uppercase'>Balance due</span>
                <span className='font-semibold'>{fmtCur(getInvoiceTotal(invoice))}</span>
              </div>
            </div>
          </section>
        </div>
