...
interface InvoicesReportViewProps {
  data: Invoice[]
...
const COLUMNS_CONFIG = [
  ...
  { id: 'username', label: 'Invoice No', minWidth: 120, filterable: true, sortable: true },
  { id: 'name', label: 'Client', minWidth: 150, filterable: true, sortable: true },
  { id: 'role', label: 'Invoice Type', minWidth: 150, filterable: true, sortable: false },
  { id: 'amount', label: 'Total Amount', minWidth: 120, filterable: true, sortable: true },
  { id: 'dueDate', label: 'Due Date', minWidth: 120, filterable: true, sortable: true },
  { id: 'status', label: 'Status', minWidth: 120, filterable: true, sortable: false },
] as const

export function InvoicesReportView({ data }: InvoicesReportViewProps) {
  ...
  const processedData = useMemo(() => {
    ...
    result = result.filter((row) => {
      ...
      const matchDueDate = filters.dueDate ? row.dueDate.toLowerCase().includes(filters.dueDate.toLowerCase()) : true
      const matchStatus = filters.status && filters.status !== 'all' ? row.status === filters.status : true

      return matchUsername && matchName && matchRole && matchAmount && matchDueDate && matchStatus
    })
    ...
    if (sortCol) {
      result.sort((a, b) => {
        ...
        if (sortCol === 'name') {
          ...
          valA = parseFloat(a.amount.replace(/[^0-9.]/g, '')) || 0
          valB = parseFloat(b.amount.replace(/[^0-9.]/g, '')) || 0
        } else {
          valA = (a[sortCol as keyof Invoice] as string || '').toLowerCase()
          valB = (b[sortCol as keyof Invoice] as string || '').toLowerCase()
        }
  ...
  const groupedData = useMemo(() => {
    ...
    processedData.forEach((row) => {
      ...
      if (groupBy === 'name') {
        ...
        const agreement = roles.find(r => r.value === row.role)
        key = agreement?.label || row.role
      } else {
        key = (row[groupBy as keyof Invoice] as string) || 'No Group Specified'
      }
          ...
          <div className='absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 px-4 py-2 bg-popover border text-popover-foreground rounded-full shadow-lg border-primary/20 animate-in fade-in slide-in-from-bottom-2 duration-150'>
            ...
            <div className='flex items-center gap-1'>
              <Button
                ...
                onClick={() => toast.success(`Newsletter successfully prepared to trigger for ${selectedCount} invoices!`)}
              >
                Send Message
              </Button>
              <Button
                      ...
                      <TableHead key='filter-status' className='px-1 py-1.5 border-r align-middle'>
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value='active' className='text-xs'>Paid</SelectItem>
                            <SelectItem value='inactive' className='text-xs'>Overdue</SelectItem>
                            <SelectItem value='invited' className='text-xs'>Draft</SelectItem>
                            <SelectItem value='suspended' className='text-xs'>Canceled</SelectItem>
                          </SelectContent>
                        </Select>
                      </TableHead>
                    )
...
const SpreadsheetRow = React.memo(function SpreadsheetRow({
  ...
  const invType = roles.find(r => r.value === row.role)
  ...
  return (
    <TableRow className={cn('h-10 group', isSelected ? 'bg-muted/20' : '')}>
      ...
      >
        <span className='capitalize truncate'>{invType?.label || row.role}</span>
      </TableCell>
      ...
      <TableCell
        onClick={() => selectCell('dueDate')}
        className={cn(getCellClasses('dueDate'), 'text-muted-foreground tabular-nums')}
      >
        {row.dueDate}
      </TableCell>
