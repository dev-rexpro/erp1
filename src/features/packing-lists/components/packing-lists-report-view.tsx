...
interface PackingListsReportViewProps {
  data: PackingList[]
...
const COLUMNS_CONFIG = [
  ...
  { id: 'username', label: 'PackingList No', minWidth: 120, filterable: true, sortable: true },
  { id: 'name', label: 'Client Account', minWidth: 150, filterable: true, sortable: true },
  { id: 'role', label: 'Packaging Format', minWidth: 150, filterable: true, sortable: false },
  { id: 'amount', label: 'Packages', minWidth: 120, filterable: true, sortable: true },
  { id: 'validUntil', label: 'Packing Date', minWidth: 120, filterable: true, sortable: true },
  { id: 'status', label: 'Status', minWidth: 120, filterable: true, sortable: false },
] as const

export function PackingListsReportView({ data }: PackingListsReportViewProps) {
    ...
    if (sortCol) {
      result.sort((a, b) => {
        ...
        if (sortCol === 'name') {
          ...
          valA = (a[sortCol as keyof PackingList] as string || '').toLowerCase()
          valB = (b[sortCol as keyof PackingList] as string || '').toLowerCase()
        }
  ...
  const groupedData = useMemo(() => {
    ...
    const groups: Record<string, PackingList[]> = {}
    processedData.forEach((row) => {
      ...
      if (groupBy === 'name') {
        ...
        key = (row[groupBy as keyof PackingList] as string) || 'No Group Specified'
      }
      ...
      <div className='relative flex flex-col flex-1 border rounded-md bg-background overflow-hidden'>
          ...
          <div className='absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 px-4 py-2 bg-popover border text-popover-foreground rounded-full shadow-lg border-primary/20 animate-in fade-in slide-in-from-bottom-2 duration-150'>
            ...
            <div className='flex items-center gap-1'>
              <Button
                ...
                onClick={() => toast.success(`PackingList ledger exported for ${selectedCount} cargo client accounts.`)}
              >
                Send PackingList PDF
              </Button>
              <Button
                variant='ghost'
                ...
                <TableRow>
                  <TableCell colSpan={COLUMNS_CONFIG.length + 1} className='py-12 text-center text-muted-foreground'>
                    <div className='flex flex-col items-center justify-center gap-2'>
                      <span className='text-sm'>No packingLists found.</span>
                      <Button
                        size='sm'
                        variant='link'
                        onClick={() =>
        ...
        <div className='flex items-center justify-between px-4 py-2 border-t bg-muted/20 font-sans text-[11px] text-muted-foreground/75'>
          <div className='flex items-center gap-2.5'>
            <span>CAPACITY: 500 packingLists</span>
  ...
  return (
    <TableRow className={cn('h-10 group', isSelected ? 'bg-muted/20' : '')}>
      ...
      <TableCell
        onClick={() => {
          selectCell('username')
          setSelectedPackingListId(row.id)
        }}
