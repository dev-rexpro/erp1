...
export function QuotationPrimaryButtons() {
  const { viewMode, setViewMode, setSelectedQuotationId } = useQuotations()
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setRefreshing(false)
      toast.success('Service quotations list refreshed!')
    }, 1000)
  }

  const handleSelectView = (mode: 'list' | 'report' | 'kanban' | 'image') => {
    if (mode === 'image') {
      toast.info('Image cards view is coming soon!')
      return
    }
    setViewMode(mode)
  }
    ...
    <div className='flex items-center gap-2'>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align='end' className='rounded-xl'>
          <DropdownMenuItem onClick={() => toast.success('Quotations bulk imported!')}>
            Bulk Import JSON
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => toast.success('Quotations bulk exported!')}>
            Export to Excel
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      <Button
        onClick={() => setSelectedQuotationId('new')}
        size='sm'
      ...
      >
        <Plus size={15} />
        <span className='text-xs'>Create Quotation</span>
      </Button>
    </div>
  )
}
